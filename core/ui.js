      ...
      for (var i = 0; i < mutations.length; i++) {
        ...
        for (var j = 0; j < addedNodes.length; j++) {
          ...
          if (addedNode.getElementsByClassName("two").length > 0) {
            ...
            addedNode.classList.contains(UIClassNames.OUTER_DROPDOWN_CLASS)
          ) {
            setTimeout(function () {
          ...
          var msgNodes = addedNode.querySelectorAll(
        ...
        for (var j = 0; j < removedNodes.length; j++) {
          ...
          if (removedNode.classList.contains("two")) {
            ...
            var menuItem = document.getElementsByClassName(
              "menu-item-incognito"
            )[0];
...
async function addIconIfNeeded() {
  ...
  var firstMenuItem = document.getElementsByClassName(
    UIClassNames.MENU_ITEM_CLASS
  )[0];
  if (firstMenuItem != undefined) {
    ...
    menuItemElem.setAttribute(
      "class",
      UIClassNames.MENU_ITEM_CLASS + " menu-item-incognito"
    );

    firstMenuItem.parentElement.insertBefore(menuItemElem, firstMenuItem);

    document.dispatchEvent(
      new CustomEvent("onOptionsUpdate", { detail: JSON.stringify(options) })
    );
    ...
    drop.close = function () {
      document.dispatchEvent(
        new CustomEvent("onIncognitoOptionsClosed", { detail: null })
      );
      setTimeout(function () {
    ...
    drop.on("open", function () {
      ...
      var pressedMenuItemClass =
        UIClassNames.MENU_ITEM_CLASS +
        " " +
        UIClassNames.MENU_ITEM_HIGHLIGHTED_CLASS +
        " active menu-item-incognito";
      document
        .getElementsByClassName("menu-item-incognito")[0]
        .setAttribute("class", pressedMenuItemClass);

      document
        .getElementById("incognito-option-read-confirmations")
        .addEventListener("click", onReadConfirmaionsTick);
      document
        .getElementById("incognito-option-online-status")
        .addEventListener("click", onOnlineUpdatesTick);
      document
        .getElementById("incognito-option-typing-status")
        .addEventListener("click", onTypingUpdatesTick);
      document
        .getElementById("incognito-option-save-deleted-msgs")
        .addEventListener("click", onSaveDeletedMsgsTick);
      document
        .getElementById("incognito-option-show-device-type")
        .addEventListener("click", onShowDeviceTypesTick);
      document
        .getElementById("incognito-option-auto-receipt")
        .addEventListener("click", onAutoReceiptsTick);
      document
        .getElementById("incognito-option-status-downloading")
        .addEventListener("click", onStatusDownloadingTick);
      for (var nextButton of document.getElementsByClassName(
        "incognito-next-button"
      )) {
        nextButton.addEventListener("click", onNextButtonClicked);
      }
      for (var nextButton of document.getElementsByClassName(
        "incognito-back-button"
      )) {
      ...
      document.dispatchEvent(
        new CustomEvent("onIncognitoOptionsOpened", { detail: null })
      );
    });
    drop.on("close", function () {
      ...
      document
        .getElementById("incognito-option-read-confirmations")
        .removeEventListener("click", onReadConfirmaionsTick);
      document
        .getElementById("incognito-option-online-status")
        .removeEventListener("click", onOnlineUpdatesTick);
      document
        .getElementById("incognito-option-typing-status")
        .removeEventListener("click", onTypingUpdatesTick);
...
function generateDropContent(options) {
  ...
  var onlineStatusCaption =
    "Stops sending presence updates. Will prevent you from seeing others' online status.";
  ...
  var readConfirmationsCaption =
    "Blocked messages will be marked with a button.";
  ...
  var showDeviceTypeCaption =
    "Shows whether each new message was sent from a phone or a computer";

  var autoReceiptTitle = "Auto-Send receipts on reply";
  var autoReceiptCaption =
    "Automatically mark messages as read when replying in a chat";
          ...
          document.dispatchEvent(
            new CustomEvent("onOptionsUpdate", {
              detail: JSON.stringify({
                showReadWarning: shouldShowReadWarning,
              }),
            })
          );
        }
...
document.addEventListener("onInterceptionWorking", function (e) {
  ...
  deletedDBOpenRequest.onsuccess = () => {
    ...
    var keys = deletedMsgsDB
      .transaction("msgs", "readonly")
      .objectStore("msgs")
      .getAll();
    keys.onsuccess = () => {
...
function onReadConfirmaionsTick() {
  ...
  var checkbox = document.querySelector(
    "#incognito-option-read-confirmations .checkbox-incognito"
  );
  ...
  if (checkbox.getAttribute("class").indexOf("unchecked") > -1) {
    ...
    var redChats = document.getElementsByClassName(
      "icon-meta unread-count incognito"
    );
...
function onOnlineUpdatesTick() {
  ...
  var checkbox = document.querySelector(
    "#incognito-option-online-status .checkbox-incognito"
  );
...
function onTypingUpdatesTick() {
  ...
  var checkbox = document.querySelector(
    "#incognito-option-typing-status .checkbox-incognito"
  );
  var checkboxClass = checkbox.getAttribute("class");
  var checkmark = checkbox.firstElementChild;
  var chekmarkClass = checkmark.getAttribute("class");
...
function onSaveDeletedMsgsTick() {
  ...
  var checkbox = document.querySelector(
    "#incognito-option-save-deleted-msgs .checkbox-incognito"
  );
  var checkboxClass = checkbox.getAttribute("class");
  var checkmark = checkbox.firstElementChild;
  var chekmarkClass = checkmark.getAttribute("class");
...
function onShowDeviceTypesTick() {
  ...
  var checkbox = document.querySelector(
    "#incognito-option-show-device-type .checkbox-incognito"
  );
...
function onAutoReceiptsTick() {
  ...
  var checkbox = document.querySelector(
    "#incognito-option-auto-receipt .checkbox-incognito"
  );
...
function onStatusDownloadingTick() {
  ...
  var checkbox = document.querySelector(
    "#incognito-option-status-downloading .checkbox-incognito"
  );
...
function onNextButtonClicked() {
  var views = Array.from(
    document.getElementsByClassName("incognito-options-view-container")
  );
...
function onSafetyDelayDisabled() {
  ...
  document.getElementById(
    "incognito-radio-enable-safety-delay"
  ).checked = false;
  document.dispatchEvent(
    new CustomEvent("onOptionsUpdate", {
...
function onSafetyDelayEnabled() {
  ...
  if (isNaN(delay))
    delay = parseInt(
      document.getElementById("incognito-option-safety-delay").placeholder
    );
  document.getElementById("incognito-option-safety-delay").disabled = false;
  document.getElementById(
    "incognito-radio-disable-safety-delay"
  ).checked = false;
  document.dispatchEvent(
    new CustomEvent("onOptionsUpdate", {
      detail: JSON.stringify({ safetyDelay: delay }),
      ...
      keys.result.forEach((value) => {
        if (value.id == msgID) {
          ...
          if (value.caption != null) {
            ...
            var textSpanStyle =
              "font-style: normal; color: rgba(241, 241, 242, 0.95); margin-top: 10px; margin-bottom: 10px;";
...
function restoreDeletedMessageIfNeeded(messageNode, msgID) {
  document.dispatchEvent(
    new CustomEvent("getDeletedMessageByID", {
      detail: JSON.stringify({ messageID: msgID }),
    })
  );
  document.addEventListener("onDeletedMessageReceived", function (e) {
    ...
    var shouldTryToSyntehesizeMessage = messageNode.textContent.includes(
      "message was deleted"
    ); // TODO: other locales?
    ...
    var messageSubElement = messageNode.getElementsByClassName(
      UIClassNames.CHAT_MESSAGE_INNER_TEXT_DIV
    )[0]; // oh well
...
function tryToSynthesizeMessage(messageSubElement, messageData) {
  ...
  if (messageData.isMedia) {
    ...
    if (messageData.type === "image") {
      ...
      imgTag.src =
        "data:" + messageData.mimetype + ";base64," + messageData.body;
...
function markMessageNodeDeviceIfPossible(messageNode, msgID) {
  ...
  document.dispatchEvent(
    new CustomEvent("getDeviceTypeForMessage", {
      detail: JSON.stringify({ messageID: msgID }),
    })
  );
  document.addEventListener("onDeviceTypeReceived", async function (e) {
    ...
    var possibleNodes = document.querySelectorAll(
      '[data-id*="' + messageID + '"]'
    );
    if (possibleNodes.length > 0) messageNode = possibleNodes[0].childNodes[0];

    if (messageNode.getElementsByClassName("device-type-image").length > 0)
      return;
    ...
    var imageElement = await generateSVGElement(
      imageURL,
      "",
      "This message was sent from a " + deviceType,
      19
    );
    ...
    if (
      topMessageNode.innerHTML.includes("chat-profile-picture") ||
      messageNode.innerHTML.includes("Open chat details")
    ) {
...
function tickCheckbox(checkbox, checkmark) {
  ...
  checkbox.setAttribute(
    "class",
    checkboxClass.replace("unchecked", "checked") + " incognito-checked"
  );
...
function untickCheckbox(checkbox, checkmark) {
  ...
  checkbox.setAttribute(
    ...
    checkboxClass
      ...
      .split("incognito-checked")
      .join(" ")
  );
  checkmark.setAttribute(
    "class",
    chekmarkClass.replace("incognito-marked", "")
  );
}
...
function isSafetyDelayValid(string) {
  ...
  return (
    (String(number) === string && number >= 1 && number <= 30) || string == ""
  );
}
