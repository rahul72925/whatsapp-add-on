...
function determineIfNodeIsStatus(node) {
  ...
  var child =
    parent?.children[1]?.children[2]?.children[0]?.children[0]?.children[0]
...
function createDownloadButton(src) {
  ...
  a.innerHTML = "<img src='" + `${CDN}/images/download.svg` + "' >";
  a.className = "download-button-incognito";
  a.href = src;
  a.id = src;
...
document.addEventListener("onOptionsUpdate", function (e) {
  ...
  if ("allowStatusDownload" in options)
    allowStatusDownload = options.allowStatusDownload;
});
